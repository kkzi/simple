...
{
public:
    void initialize(HICON icon, std::initializer_list<tray_menu> menus, HWND hwnd = 0)
    {
        menus_ = menus;
        hwnd_ = hwnd > 0 ? hwnd
                         : create_window(create_window_option{
                               .inst = GetModuleHandle(NULL),
                               .title = str::wide(WC_TRAY_CLASS_NAME),
                               .cmdshow = SW_HIDE,
                               .process = tray_window_proc,
                           });
        init_tray(icon);
        update_tray();
    }
...
private:
    HWND init_hwnd()
    {
        ...
        auto hwnd = CreateWindowEx(0, TEXT(WC_TRAY_CLASS_NAME), NULL, 0, 0, 0, 0, 0, 0, 0, 0, 0);
        if (hwnd == NULL)
        {
            return 0;
        }
